...
namespace HIQTrainingSite.ViewModel {
    public class CertificationSearchViewModel {

        public CertificationSearchViewModel() {
            Companies = new CompanyDetailListViewModel();
            Companies.SelectedCompanyId = -1;
            Status = -1; // all
        }

        public CertificationSearchViewModel(string name, int? companyId, int? status) {
            Name = name;
            Companies = new CompanyDetailListViewModel();
